...
Владелец может открыть «Дополнить задачу» из кабинета или карточки до подтверждения выбора команд. Форма пересчитывает рейтинг локально; единственный PATCH /api/tasks/:id отправляется после ручного подтверждения. Метод адаптера updateTask(id, draft) не меняет указатель текущего черновика. ID, дата публикации и отклики сохраняются. Сервер запрещает пустое название/контекст и изменения после выбора команд. Отмена возвращает прежнюю карточку.
