...
(() => {
  window.createMatchUI = ({ api, app, state, h, field, toast, go }) => {
    ...
    const choices = (name, label, values, value) => `<label class="field">${label}<select name="${name}"><option value="">Не указано</option>${values.map(v => `<option value="${v}" ${v === value ? 'selected' : ''}>${({Beginner:'Начальный',Intermediate:'Средний',Advanced:'Продвинутый',Easy:'Простая',Medium:'Средняя',Hard:'Сложная'})[v]||v}</option>`).join('')}</select></label>`;
    ...
    const slot = task => `<section class="match-box" data-match-task="${h(task.id)}" aria-label="Соответствие задаче"><div class="match-skeleton" role="status"><span></span><div><i></i><i></i></div><span class="visually-hidden">Рассчитываем соответствие…</span></div></section>`;
    async function attach(tasks) {
      ...
      try {
        ...
        await Promise.all(nodes.map(async node => {
          ...
          if (!task) return;
          if (task.matchAvailable === false) { if(valid(node)) node.innerHTML='<p class="muted">Заполните профиль команды для расчёта соответствия</p><a class="btn secondary small" href="#profile">Добавить навыки и уровень ↗</a>'; return; }
          try {
            ...
            node.innerHTML = summary(match, key) + `<div class="match-card-actions"><a href="#profile">Настроить профиль</a>${task.status === 'published' && task.canApply !== false ? `<button type="button" class="btn small" data-match-apply="${h(task.id)}">Предложить решение →</button>` : '<span class="muted">Приём завершён</span>'}</div>`;
    ...
    function offer(match, student, task, id) {
      if (!match || !student) return '<div class="match-box match-empty">' + (student ? 'Расчёт недоступен: уточните навыки, сложность и нагрузку задачи.' : 'Профиль команды не приложен к этому отклику.') + '</div>';
      const key = 'offer:' + id;
      records.set(key, { match, student, task: match.task || task, snapshot: true });
      return `<section class="match-box">${summary(match, key, student.name || 'Участник команды')}<p class="match-caveat">Состав и навыки команды на момент отклика</p>${student.members?.length?`<ul class="team-snapshot">${student.members.map(m=>`<li><strong>${h(m.name)}</strong> · ${h(m.role)}<br>${h(m.skills.join(', '))}</li>`).join('')}</ul>`:''}</section>`;
    }
    ...
    function showDetails(key) {
      ...
      const labels = { skills: 'Навыки', experience: 'Опыт', interest: 'Интересы', difficulty: 'Сложность', availability: 'Доступное время' };
      dialog.innerHTML = `<div class="match-dialog-head"><div><span class="eyebrow muted">Соответствие · ${h(student.name || 'Ваш профиль')}</span><h2 id="match-dialog-title">Почему задача подходит команде</h2><p class="muted">${h(task.title || 'Задача')}</p></div><button class="dialog-close" type="button" aria-label="Закрыть подробности">×</button></div><div class="match-dialog-score">${ring(match)}<div><h3>${match.matchScore}% соответствия</h3><span>${h(match.label)}</span><p class="muted">${record.snapshot ? 'Снимок на момент отправки предложения' : 'Расчёт по сведениям профиля'}</p></div></div><div class="match-breakdown">${Object.entries(labels).map(([k,label]) => `<div><div><strong>${label}</strong><span>${match.breakdown[k]}% <small>× ${match.weights[k]}% веса</small></span></div><div class="bar"><span style="width:${match.breakdown[k]}%"></span></div><small>Вклад: ${match.contributions[k]} из ${match.weights[k]} баллов</small></div>`).join('')}</div><p class="match-caveat">Итог — взвешенная сумма пяти критериев, округлённая до целого. Неизвестные сведения не дают баллов. Соответствие не является решением о выборе команды.</p><div class="match-detail-columns"><section><h3>Сильные стороны</h3><ul class="match-reasons">${match.strengths.map(s => `<li>✓ ${h(s)}</li>`).join('') || '<li>Пока недостаточно сведений</li>'}</ul></section><section><h3>Недостающие навыки</h3>${skillChips({ matchedSkills: [], missingSkills: match.missingSkills })}${!match.missingSkills.length ? `<p class="muted">${match.task?.requiredSkills?.length ? 'Нет пропусков среди указанных навыков' : 'Навыки задачи не указаны'}</p>` : ''}</section></div><section class="match-recommendations"><h3>Как повысить соответствие</h3><ul>${match.recommendations.map(s => `<li>${h(s)}</li>`).join('') || '<li>Поддерживайте профиль и примеры работ в актуальном состоянии.</li>'}</ul></section><div class="match-explanation"><span class="eyebrow muted" id="match-explanation-source">Объяснение по критериям</span><p id="match-explanation-text">${h(match.explanation)}</p>${api.meta.mode === 'live' ? '<button class="text-link" id="explain-match" type="button">✦ Объяснить с OpenAI</button>' : '<small>Демо использует тот же алгоритм без OpenAI.</small>'}<p class="muted" id="match-explanation-notice"></p></div><p class="match-caveat">Навыки, GitHub-технологии и личные проекты указаны участником. Ссылки автоматически не проверяются. Подтверждённые этапы платформы также учитываются в опыте.</p>`;
    ...
    async function renderProfile(ticket) {
      if (state.role !== 'student') { app.innerHTML = '<div class="empty"><h2>Профиль команды</h2><p>Переключите роль на «Студенческая команда» в шапке.</p></div>'; return; }
      ...
      const read = () => {
        ...
        return { members:members.map((_,i)=>({name:result['member-name-'+i],role:result['member-role-'+i],skills:split(result['member-skills-'+i]),level:result['member-level-'+i],availabilityHours:result['member-hours-'+i]})), name: result.name, skills: split(result.skills), experience: String(result.experience || '').split('\n').filter(Boolean), interests: split(result.interests),
      ...
      function draw() {
        ...
        if(members.length){for(const key of ['skills','level','availabilityHours']){const input=form.elements.namedItem(key);input.disabled=true;}}
        form.querySelectorAll('input[name^="member-hours-"]').forEach(input=>{input.min='0';input.max='168';input.step='0.5';});
        document.querySelector('#add-team-member').onclick=()=>{value={...value,...read()};members=value.members;projects=value.projects;if(members.length>=20){toast('Не больше 20 участников');return;}members.push({name:'',role:'',skills:[],level:'',availabilityHours:null});draw();};
        document.querySelectorAll('[data-remove-member]').forEach(button=>button.onclick=()=>{value={...value,...read()};members=value.members;projects=value.projects;members.splice(Number(button.dataset.removeMember),1);draw();});
        ...
        form.onsubmit = async event => {
          ...
          try {
            ...
            document.querySelector('#profile-error').textContent = ''; toast('Профиль сохранён. Соответствие пересчитается при открытии задач.');
        ...
        document.querySelector('#add-profile-project').onclick = () => {
          value = { ...value, ...read() }; projects = value.projects; members = value.members || members;
        ...
        document.querySelectorAll('[data-remove-project]').forEach(button => button.onclick = () => { value = { ...value, ...read() }; projects = value.projects; members=value.members||members; projects.splice(Number(button.dataset.removeProject), 1); draw(); document.querySelector('#profile-save-status').textContent = 'Есть несохранённые изменения'; });
        document.querySelector('#fill-match-demo').onclick = () => { value = JSON.parse(JSON.stringify(window.MostMatchDemo.student)); members=[]; projects = value.projects; draw(); document.querySelector('#profile-save-status').textContent = 'Пример заполнен — сохраните профиль'; };
      }
      draw();
    }
    async function demo() {
