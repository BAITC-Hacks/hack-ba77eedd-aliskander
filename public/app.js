...
(() => {
  ...
  async function createPage(step, ticket) {
    ...
    if (!state.draft) state.draft = await api.getDraft() || { problem: '', title: '', company: api.meta.mode === 'demo' ? 'Кофейня «Зёрна»' : '', users: '', contact: '', category: 'Аналитика', outcome: '', success: '', data: '', deadline: '', constraints: '' };
