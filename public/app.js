...
(() => {
  ...
  const url = value => { try { const parsed = value.startsWith('/') ? new URL(value,location.href) : new URL(value); return ['https:', 'http:'].includes(parsed.protocol) ? h(parsed.href) : ''; } catch (_) { return ''; } };
  ...
  function ratingPanel(rating, editable = false) {
    ...
    return `<div class="eyebrow muted">Готовность задачи</div>
      ...
      <p class="muted rating-note">Баллы показывают полноту сведений. AI даёт отдельные рекомендации по качеству и не меняет рейтинг.</p>`;
  }
  ...
  function published(task) {
    app.innerHTML = `<div class="intro"><div class="eyebrow muted">Готово к новым решениям</div><h1 class="page-title" style="margin-top:16px">Ваша задача уже в каталоге</h1><p class="muted">Команды могут изучить карточку и предложить свой подход. Решение о сотрудничестве остаётся за вами.</p></div>${steps(4)}<div class="layout"><div class="panel"><span class="tag">Опубликована</span><h2 style="margin-top:20px">${h(task.title)}</h2><p class="muted">${h(task.company)} · ${h(task.category)}</p><div class="notice">Рейтинг готовности: <strong>${task.rating.total} из 100</strong>. Команды найдут задачу через поиск, фильтры и персональные рекомендации.</div><div class="actions"><a class="btn" href="#task/${encodeURIComponent(task.id)}">Посмотреть задачу ↗</a><a class="btn secondary" href="#workspace">Мой кабинет</a></div></div><aside class="panel">${ratingPanel(task.rating)}</aside></div>`;
  }
  const editLink = task => task.status === 'published' && !task.selectionDone ? `<a class="btn secondary small" href="#edit-published/${encodeURIComponent(task.id)}">Дополнить задачу</a>` : '';
  async function editPublished(task, ticket) {
    const owner = api.meta.mode === 'demo' ? 'business-1' : window.platformAuth?.user?.id;
    if (state.role !== 'business' || task.ownerId !== owner || task.status !== 'published' || task.selectionDone) throw new Error('Редактирование доступно владельцу до подтверждения выбора команд.');
    const draft = {...task};
    const before = task.rating.total;
    app.innerHTML = `<a class="back" href="#task/${encodeURIComponent(task.id)}">← К карточке</a><h1 class="page-title">Дополните опубликованную задачу</h1><p class="muted">До подтверждения каталог показывает прежнюю версию. После сохранения рейтинг и порядок при сортировке по готовности обновятся. Отклики команд сохранятся.</p><div class="editor-layout"><section class="panel"><form id="published-edit-form">${window.MostReviewEditor(draft,{field,categoryField,h})}<label class="check-label publish-check"><input name="confirmed" type="checkbox" required>Я проверил(а) сведения и подтверждаю изменения в общем каталоге.</label><div class="actions"><button class="btn" type="submit">Подтвердить изменения</button><a class="btn secondary" href="#task/${encodeURIComponent(task.id)}">Отмена</a></div><p id="edit-status" role="status">Изменения ещё не сохранены.</p></form></section><aside class="panel"><p>Рейтинг до изменений: <strong>${before} из 100</strong></p><div id="edit-rating"></div></aside></div>`;
    const form = document.querySelector('#published-edit-form');
    // The published editor has no AI generation or draft autosave side effects.
    form.querySelector('#quality-ai')?.closest('details')?.remove();
    ...
    const capture = () => {
      ...
      const count=Number(draft.recommendedTeamSize);draft.teamSize=count===1?'1':count>=2&&count<=3?'2-3':count>=4&&count<=5?'4-5':count>5?'5+':'';
    ...
    const rate = async () => {
      ...
      try {const rating=await api.rateTask({...draft});if(ticket===state.ticket&&current===version)document.querySelector('#edit-rating').innerHTML=ratingPanel(rating,true);}
      catch(error){if(ticket===state.ticket&&current===version)document.querySelector('#edit-rating').textContent=error.message;}
    };
    form.oninput = event => {if(event.target.name==='confirmed')return;capture();form.elements.namedItem('confirmed').checked=false;rate();};
    document.querySelector('#edit-rating').onclick = event => {
      const key=event.target.closest('[data-focus-field]')?.dataset.focusField;
      const target=key&&form.elements.namedItem(key);if(target){const details=target.closest('details');if(details)details.open=true;target.focus();target.scrollIntoView({behavior:'smooth',block:'center'});}
    };
    form.onsubmit = event => {
      event.preventDefault();
      action(event.submitter,async()=>{
        capture();if(!form.reportValidity()||!form.elements.namedItem('confirmed').checked)return;
        ...
        form.inert=true;
        try {const updated=await api.updateTask(task.id,draft);toast(`Карточка обновлена. Рейтинг: ${before} → ${updated.rating.total} из 100.`);go('task/'+encodeURIComponent(task.id));}
  ...
  function offersPage(task, offers) {
    if (state.role !== 'business' || task.ownerId !== (api.meta.mode === 'demo' ? 'business-1' : window.platformAuth?.user?.id)) { app.innerHTML = empty('Предложения доступны владельцу задачи', 'Переключитесь в кабинет бизнеса для своей задачи.', '<a class="btn secondary" href="#workspace">Открыть кабинет</a>'); return; }
  ...
  async function render() {
    ...
    document.querySelector('#team-profile').hidden = api.meta.mode !== 'demo' || state.role !== 'student';
    ...
    try {
      if (api.meta.mode === 'live' && (['login','register'].includes(page) || (!window.platformAuth.user && !['catalog','catalog-list','task'].includes(page)))) window.platformAuth.render(app,h,page==='register');
      else if (page === 'catalog' || page === 'catalog-list') await catalogPage.render(query,ticket);
      else if (page === 'edit-published') {
        const task = await api.getTask(decodeURIComponent(param || ''));
        if (ticket === state.ticket) await editPublished(task,ticket);
      }
      ...
      else if (page === 'defense-case' && api.meta.mode==='demo'){const task=await api.startCase();state.role='business';roleSelect.value='business';try{localStorage.setItem('most-platform-role','business');}catch{}go('task/'+task.id);}
  ...
  if (api.meta.mode === 'live') window.platformAuth.init().then(user=>{state.role=user?.role||'guest';roleSelect.value=state.role;render();}).catch(error=>{app.textContent='Не удалось проверить вход: '+error.message;});
  else render();
})();
