...
(() => {
  ...
  const categories = ['Аналитика', 'Веб-разработка', 'Frontend', 'Backend', 'Fullstack', 'Mobile', 'Дизайн', 'ИИ и автоматизация', 'Исследования'];
  ...
  function card(task) {
    return `<article class="card task-catalog-card"><div class="card-top"><span class="tag ${task.category === 'Дизайн' ? 'orange' : task.category === 'Веб-разработка' ? 'blue' : ''}">${h(task.category)}</span><span class="ready">● ${task.rating.total}% готовности</span></div><h3><a href="#task/${encodeURIComponent(task.id)}">${h(task.title)}</a></h3><p>${h(task.problem.length > 135 ? task.problem.slice(0, 135) + '…' : task.problem)}</p><div class="company"><span class="company-icon">${h(task.company.charAt(0))}</span>${h(task.company)}</div>${state.role === 'student' ? matchUI.slot(task) : ''}<div class="card-bottom"><span>${h(task.deadline || 'Срок обсуждается')} · ${h(taskStatus[task.status])}</span><a href="#task/${encodeURIComponent(task.id)}" class="arrow" aria-label="Открыть задачу: ${h(task.title)}">↗</a></div></article>`;
  }
  function renderCards(tasks) {
    ...
    if (state.role === 'student') matchUI.attach(found);
  }
  ...
  async function render() {
    ...
    try {
      ...
      else if (page === 'match-demo') { state.role = 'student'; roleSelect.value = 'student'; await matchUI.demo(); }
